...
Most reference managers are able to do this automatically - [Zotero](https://www.zotero.org) with the [Better BibTex plug-in](https://retorque.re/zotero-better-bibtex/) works best, but I personally use [Mendeley](https://www.mendeley.com)).
...
We can also use **hyperlink syntax** and add \# before the label:
...
So Figure \@ref(fig:captain) is [this image](#captain).

And in Figure \@ref(fig:cars-plot) we saw a [cars plot](#cars-plot).
...
So Table \@ref(tab:cars-table2) is [this table](#cars-table2).

And in Table \@ref(tab:cars-table) we saw more or less [the same cars table](#cars-table).
...
A problem here is that LaTeX commands won't display in HTML output, so in the gitbook output we'd see simply "Figure \@ref(fig:cars-plot) on page ".

A not super elegant way to get around this is to insert the text with inline R code, in which we first make a check of the output format and then insert the appropriate text.

- So this: `` ``r ''`r ifelse(knitr::opts_knit$get('rmarkdown.pandoc.to') == 'latex', "Figure \\@ref(fig:cars-plot) on page \\pageref{fig:cars-plot}", "")` ``
- Inserts this (check this on both PDF and gitbook): `r ifelse(knitr::opts_knit$get('rmarkdown.pandoc.to') == 'latex', "Figure \\@ref(fig:cars-plot) on page \\pageref{fig:cars-plot}", "")`

Note that we need to escape the backslash with another backslash here to get the correct output.

A slightly less verbose way of doing this is to add a code chunk in which we assign the value of `knitr::opts_knit$get('rmarkdown.pandoc.to') == 'latex'` to a short variable, like this:

```{r}
is_pdf <- knitr::opts_knit$get('rmarkdown.pandoc.to') == 'latex'
```

Then we could less verbosely write `` ``r ''`r ifelse(is_pdf, "Figure \\@ref(fig:cars-plot) on page \\pageref{fig:cars-plot}", "")` ``, to get `r ifelse(is_pdf, "Figure \\@ref(fig:cars-plot) on page \\pageref{fig:cars-plot}", "")`.
