...
```{r oxfordlogo, fig.cap="Oxford logo", out.width='60%', fig.align='center'}
...
Lastly, we will explore how to rotate figures using the `out.extra` chunk option. The syntax is different for LaTeX and HTML, so for ease we might start by assigning the right string to a variable that will first check what format we're outputting to:

```{r define-out-extra, echo=TRUE}
...
Then we can refer to that in our chunk options to rotate a figure, as shown in Figure \@ref(fig:subd2).

```{r subd2, results="asis", echo=FALSE, out.extra=rotate180, fig.cap="A Larger Figure, Flipped Upside Down", out.width='60%'}
include_graphics("figures/subdivision.png")
```
