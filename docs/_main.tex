...
Highlight \textbf{short, inline corrections} by doing \texttt{{[}like\ this{]}\{.correction\}} --- the text between the square brackets will then \ctext[HTML]{CCCCFF}{be highlighted in blue} in the output.

This should work, also with \ctext[HTML]{CCCCFF}{cross references (do you remember chapter \ref{rmd-basics}?)} or even \ctext[HTML]{CCCCFF}{citations \autocite{Shea2014}}.
...
If all else fails in such cases, use LaTeX syntax directly (this has the drawback of not showing up in HTML output, but you probably only care about correction highlighting in the PDF version for your examiners anyway!):

\begin{quote}
\texttt{It\ doesn\textquotesingle{}t\ get\ confused\ by\ \textbackslash{}ctext{[}HTML{]}\{CCCCFF\}\{this\ \textbackslash{}textcite\{Shea2014\}\}}~\\
Becomes: It doesn't get confused by \ctext[HTML]{CCCCFF}{this \textcite{Shea2014}}.
\end{quote}
