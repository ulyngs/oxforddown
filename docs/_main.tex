...
\begin{Shaded}
\begin{Highlighting}[]
\NormalTok{knitr}\SpecialCharTok{::}\FunctionTok{include\_graphics}\NormalTok{(}\StringTok{"figures/beltcrest.png"}\NormalTok{)}
\end{Highlighting}
\end{Shaded}
...
\hypertarget{control-figure-placement}{%
\subsubsection{Control figure placement}\label{control-figure-placement}}
...
\begin{Shaded}
\begin{Highlighting}[]
\NormalTok{knitr}\SpecialCharTok{::}\FunctionTok{include\_graphics}\NormalTok{(}\StringTok{"figures/captain.jpeg"}\NormalTok{)}
\end{Highlighting}
\end{Shaded}
...
{\centering \includegraphics[width=0.5\linewidth]{/Users/ulriklyngs/Library/Mobile Documents/com~apple~CloudDocs/Work/stored-oxford-dphil-stuff/UL DPhil at SOCIAM/Thesis/formatting_considerations/oxforddown/figures/latex_font_sizes} 
...
\hypertarget{notes-on-the-oxthesis-template-and-on-collaboration}{%
\chapter{Notes on The OxThesis template and on collaboration}\label{notes-on-the-oxthesis-template-and-on-collaboration}}
...
To make chapters unnumbered (this is normally only relevant to the Introduction and/or the Conclusion), follow the chapter header with \texttt{\{-\}}, e.g.~\texttt{\#\ Introduction\ \{-\}}.
...
In such cases, you could just use LaTeX syntax directly (this has the drawback of not showing up in HTML output, but you probably only care about correction highlighting in the PDF version for your examiners anyway!).
The correction highlighting is supplied by the \href{https://ctan.org/pkg/soul}{soul} package, so you would do like this:
...
This chapter describes a number of additional tips and tricks as well as possible customizations to the \texttt{oxforddown} thesis.

\hypertarget{applying-custom-font-color-and-highlighting-to-text}{%
\section{Applying custom font color and highlighting to text}\label{applying-custom-font-color-and-highlighting-to-text}}

The lua filter that adds the functionality to highlight corrections adds two more tricks:
you can apply your own choice of colour to highlight text, or to change the font color of some text.
The syntax is as follows:

\begin{quote}
Here's \texttt{{[}some\ text\ in\ green\ highlighting{]}\{highlight="green"\}}\\
Becomes: Here's \sethlcolor{green}\hl{some text in green highlighting}\sethlcolor{correctioncolor}.
\end{quote}

\begin{quote}
\texttt{{[}Here\textquotesingle{}s\ some\ text\ with\ red\ font{]}\{color="red"\}}~\\
Becomes: \textcolor{red}{Here's some text with red font}
\end{quote}

\begin{quote}
Finally --- never, ever actually do this -- \texttt{{[}here\textquotesingle{}s\ some\ text\ with\ green\ highlighting\ and\ red\ font{]}\{highlight="green"\ color="red"\}}\\
Becomes: \textcolor{red}{\sethlcolor{green}\hl{here's some text with green highlighting and red font}\sethlcolor{correctioncolor}}
\end{quote}
...
\begin{Shaded}
\begin{Highlighting}[]
\NormalTok{knitr}\SpecialCharTok{::}\FunctionTok{include\_graphics}\NormalTok{(}\StringTok{"figures/pdf\_example/Lyngs2020\_FB.pdf"}\NormalTok{)}
\end{Highlighting}
\end{Shaded}
...
\CommentTok{\# split PDF into pages stored in figures/pdf\_example/split/}
\CommentTok{\# pdftools::pdf\_split("figures/pdf\_example/Lyngs2020\_FB.pdf",}
\CommentTok{\# output = "figures/pdf\_example/split/")}

\CommentTok{\# grab the pages}
\NormalTok{pages }\OtherTok{\textless{}{-}} \FunctionTok{list.files}\NormalTok{(}\StringTok{"figures/pdf\_example/split"}\NormalTok{, }\AttributeTok{full.names =} \ConstantTok{TRUE}\NormalTok{)}
...
If you are a LaTeX expert and you need further customisation that what's currently provided, you can tweak the relevant sections of \textbf{templates/template.tex} - the relevant code is beneath the line that begins \texttt{\textbackslash{}usepackage\{fancyhdr\}}.
