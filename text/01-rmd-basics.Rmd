...
```
<!-- Note a gotcha in the savequote block: for the qauthor, you have to use LaTeX syntax directly, and when doing so, you must also use LaTeX instead of Markdown syntax to, for example, italicise something inside \qauthor{}

include=knitr::opts_knit$get('rmarkdown.pandoc.to') == 'latex' ensures that it's only shown when output is latex
