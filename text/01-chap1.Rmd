...
<!-- Note the little gotcha in the savequote block here: if you want to note the author, you have to use LaTeX syntax directly with \qauthor{}, and when you do that, you must also use LaTeX formatting syntax if you want to, for example, italicise something -->
...
<!-- UL: this works with echo = FALSE. Find way to actually embed code chunks. -->
```{r cars}
summary(cars)
```
...
You can also embed plots.  For example, Figure \@ref(fig:pressure) shows a way to use the base **R** graphics package to produce a plot using the built-in `pressure` dataset.

```{r pressure, echo=FALSE, cache=TRUE, fig.cap="A dynamically generated plot using the built-in 'pressure' dataset"}
plot(pressure)
```
...
We see that the flight occurred on March 3rd and departed a little after 2 PM on its way to Dallas/Fort Worth.  Lastly, Figure \@ref(fig:march3plot) shows how we can visualize the arrival delay of all departing flights from Portland on March 3rd against time of departure.

```{r march3plot, fig.height=3, fig.width=6, fig.cap="Arrival delay of departing flights from Portland on March 3rd."}
...
This document introduction won't serve as a complete primer on \LaTeX.  There are plenty of those online, and googling your questions will often get you answers, especially from <http://tex.stackexchange.com>.
...
Within months of Röntgen's discovery of the X-ray in \mccorrect{1895}[@hipr1997], cardiac pathology was being investigated via non-invasive imaging [@goochandgooch2001].  Over the intervening years, cardiac imaging modalities and techniques have advanced significantly.  Clinically, cardiac imaging is used for two broad purposes: diagnosis of pathophysiology and guidance of interventional procedures.  These applications impose different requirements on imaging equipment, image acquisition time, computational complexity, spatial and temporal resolution, and tissue discrimination.  The common diagnostic and interventional cardiac imaging techniques in current clinical practice are reviewed below.  An accessible introduction to the physics of medical imaging can be found in Webb's *Introduction to Biomedical Imaging* [@goochandgooch2001a].  A comprehensive overview of the use of imaging in clinical cardiology is presented in Leeson's *Cardiovascular Imaging* [@hertzmann2000].
...
```{r fourchamber, out.width='0.7\\textwidth', fig.cap="Four-chamber illustration of the human heart.  Clockwise from upper-left: right atrium, left atrium, left ventricle, right ventricle.", echo=FALSE}
knitr::include_graphics("figures/sample/Gray498.png")
```

The use of acoustic waves for medical diagnosis, inspired by naval sonar, was initially developed in the 1940s [@jain1989].  By 1954, the first clinically useful cardiac ultrasound -- examining motion of the mitral valve in stenosis -- was reported [@Molina1994].  These early scans were one-dimensional images ('A-mode'), sometimes repeated to generate a time axis ('M-mode'). The sector-scanning probe was developed in the 1970s [@noble2002], leading to the `B-mode' that a modern cardiologist would recognise as an echocardiogram.