...
In other words, `<span class="correction">do like this</span>`, which in <span class="correction">the output will be highlighed in blue</span>.
To stop inline corrections from being highlighted in PDF output, comment out `pandoc_args: ["--filter", "correction_filter.py"]` under `bookdown::pdf_book` in the YAML header. 
