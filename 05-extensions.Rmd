...
```{r, eval=knitr::is_html_output(), out.width='100%', out.height='1000px', fig.cap="A PDF paper inserted in the gitbook with knitr::include_graphics"}
knitr::include_graphics("figures/pdf_example/Mensh_Kording_2017.pdf")
```

In LaTeX output, however, this approach can cause odd behaviour.
Therefore, when you build your thesis to PDF, split the PDF into an alphanumerically sorted sequence of **single-page** PDF files (you can do this automatically with the package `pdftools`) and then use the appropriate LaTeX command to insert them, as shown below.
...
# #split PDF in pages stored in figures/pdf_example/split/
# pdftools::pdf_split("figures/pdf_example/Mensh_Kording_2017.pdf", 
...
# add initial page break to place first pdf on a new page
cat('\\newpage')

# for each PDF page, insert it nicely and
# end with a page break
cat(stringr::str_c("\\begin{center} \\makebox[\\linewidth][c]{\\includegraphics[width=",
           pdf_width,
           "\\linewidth]{",
           pages, 
           "}} \\end{center} \\newpage"))
